...
if(isset($_POST["recover-submit"]))
{
  ...
  $check1 = "SELECT * from superuser WHERE Super_Email = '$forget_email'";
  $result1 = mysqli_query($connect, $check1);
  $check2 = "SELECT * from admin WHERE Adm_Email = '$forget_email' and Adm_isDelete = 0";
  ...
  if(mysqli_num_rows($result2)>0 || mysqli_num_rows($result1)>0)
  {
    ...
    if(mysqli_num_rows($result1)>0 && mysqli_num_rows($result2)==0){
      $string = $row1['Super_ID'];
    }else if(mysqli_num_rows($result2)>0 && mysqli_num_rows($result1)==0){
      $string = $row2['Adm_ID'];
    }
    

    $encryption_key = substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'),1,12);
    $user_id = EncryptThis($string,$encryption_key);
    ...
    $headers .= "From: ".$from_email."\r\n".
      ...
      if(mysqli_num_rows($result1)>0 && mysqli_num_rows($result2)==0){
        $body = "<html><body>";
        $body .= "<h3>Dear Superuser <span style='text-transform: uppercase';>".$row1['Super_Name']."</span></h3>".
                "<p>We have received your request to reset your password.<br>".
                "Please proceed to ".$CurPageURL."/fiesta/admin/reset-pass.php?sid=".$user_id."&ei=".$encryption_key." to reset your password.</p>".
                "<p>Regards,<br>Fiesta Corp. Customer Service</p>";
        $body .= "</body></html>";
      }else if(mysqli_num_rows($result2)>0 && mysqli_num_rows($result1)==0){
        $body = "<html><body>";
        $body .= "<h3>Dear Admin <span style='text-transform: uppercase';>".$row2['Adm_Name']."</span></h3>".
                "<p>We have received your request to reset your password.<br>".
                "Please proceed to ".$CurPageURL."/fiesta/admin/reset-pass.php?id=".$user_id."&ei=".$encryption_key." to reset your password.</p>".
                "<p>Regards,<br>Fiesta Corp. Customer Service</p>";
        $body .= "</body></html>";
      }
