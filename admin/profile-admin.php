                ...
                <div class="col-md-4">
						<div class="prof-pic">
							<div class="prof-pic-col">
								<img src="../<?php echo $row['Adm_Photo']?>" class="display-photo"/>
							</div>
						</div>
					</div>
					<div class="col-md-8">
						<div class="profile-body">
							<div class="tab-content" id="myTabContent">
								...
								<div class="tab-pane fade show active" id="view-profile" role="tabpanel" aria-labelledby="view-profile-tab">
									...
									<div class="user-info">
										<div class="form-group" style="color:black;"><p class="label" >Name</p><p><?php echo $row['Adm_Name']?></p></div>
										<div class="form-group" style="color:black;"><p class="label">Email</p><p><?php echo $row['Adm_Email']?></p></div>
										<div class="form-group" style="color:black;"><p class="label">Phone</p><p><?php echo $row['Adm_PhoneNo']?></p></div>
										<div class="form-group" style="color:black;"><p class="label">Staff ID</p><p><?php echo $row['Adm_Staff_ID']?></p></div>
									</div>
									...
									<div class="edit-user-info">
										<form class="form-submit" id="edit-profile" method="post" name="edit_profile" enctype="multipart/form-data">
											<div class="form-group" id="user-name">
												<input type="text" name="user-name" class="" placeholder="Name" value="<?php echo $row['Adm_Name']?>" id="nm" onfocus="name_focus()" onblur="name_blur()" onkeyup="name_keyup()" maxlength="255">
											...
											<div class="form-group" id="user-email">
												<input type="email" name="user-email" class="" placeholder="Email" value="<?php echo $row['Adm_Email']?>" id="email" onfocus="email_focus()" onblur="email_blur()" onkeyup="email_keyup()" maxlength="255" readonly>
											...
											<div class="form-group" id="user-phone">
												<input type="text" name="user-phone" class="" placeholder="Phone Number" value="<?php echo $row['Adm_PhoneNo']?>" id="phone" onfocus="phone_focus()" onblur="phone_blur()" onkeyup="phone_keyup()" maxlength="12">
											...
											<div class="form-group" id="user-staff-id">
												<input type="text" name="user-staff-id" class="" placeholder="Staff ID" value="<?php echo $row['Adm_Staff_ID']?>" id="staffid" readonly>
											</div>
											<div class="prof-pic-upload-col">
												<input type="file" name="adminPic" class="btn-upload-prof-pic" accept="image/*"/>
											</div>
...
<?php
	//check the data is from which form
	if(isset($_POST["formID"]) && $_POST["formID"] == 1)
	{
		...
		{
		$path = $_FILES['adminPic']['name'];
		$ext = pathinfo($path, PATHINFO_EXTENSION);
		$adminPic = "images/admin/"."(".date('dmYHis').")".$new_name.".".$ext;
		}
		else
		{
		  $adminPic =  $row["Adm_Photo"];
		}
		$newFileDir = '../'.$adminPic;

		$check = "SELECT * from admin WHERE Adm_ID != '$user_id' and Adm_Email = '$new_email' and Adm_isDelete = 0";
		$result = mysqli_query($connect, $check);

		$sql= "UPDATE admin SET Adm_Name = '$new_name', Adm_Email = '$new_email', Adm_PhoneNo = '$new_phone', Adm_Photo = '$adminPic' WHERE Adm_ID='$user_id' and Adm_isDelete = 0";
		...
		else
		{
			if(mysqli_query($connect,$sql))
			{
				...
				swal({
					...
					location.replace("profile-admin.php");
				});
				</script>
				<?php
			
			if(move_uploaded_file($_FILES['adminPic']['tmp_name'],$newFileDir)){
	...
	if(isset($_POST["formID"]) && $_POST["formID"] == 2)
	{
		...
		$result = mysqli_query($connect, "SELECT * FROM admin WHERE Adm_ID='$user_id' and Adm_Password = PASSWORD('$cur_pass') and Adm_isDelete = 0");
		...
		{
			$result = mysqli_query($connect, "SELECT * FROM admin WHERE Adm_ID='$user_id' and Adm_Password = PASSWORD('$new_pass') and Adm_isDelete = 0");
			$count = mysqli_num_rows($result);
			if($count>0)
			{
				?>
				<script>
					swal({
						title: "Password Same",
						text: "Your New Password Is Same As Old Password!!!",
						icon: "warning",
						button: "Continue",
					}).then(function() {
						location.replace("profile-admin.php");
			...
			{
				$sql = "UPDATE admin SET Adm_Password = PASSWORD('$new_pass') WHERE Adm_ID='$user_id' and Adm_isDelete = 0";
